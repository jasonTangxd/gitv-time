...
public class LIV_Distribute_Bolt implements IRichBolt {
    /**
     *
     */
    ...
    private Logger log = LoggerFactory.getLogger(LIV_Distribute_Bolt.class);
...
//            String play_type = input.getStringByField("play_type");
            ...
            if (!StringHandle.isLegalField(partner, mac, albumName, timeLength, playLength, chnCode, logDate, srcId)) {
                return;
            }
            String srcName = null;
            ...
            if (srcName != null) {
                collector.emit(Constant.LIV_TO_USER, new Values(partner, mac, srcName, chnCode, albumName, playOrder,
                        play_Length, time_Length, logDate));
                collector.emit(Constant.LIV_TO_PROGRAM, new Values(partner, srcName, chnCode, albumName, playOrder,
            ...
            log.error("LIV_Distribute_Bolt exception--{}", e.getMessage());
        } finally {
            collector.ack(input);
        }
    ...
    public void declareOutputFields(OutputFieldsDeclarer declarer) {
        declarer.declareStream(Constant.LIV_TO_USER, new Fields("partner", "mac", "srcName", "chnCode", "albumName",
                "playOrder", "playLength", "timeLength", "logDate"));
        declarer.declareStream(Constant.LIV_TO_PROGRAM, new Fields("partner", "srcName", "chnCode", "albumName", "playOrder",
