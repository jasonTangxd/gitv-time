...
public class LIV_ToUser_Update_Bolt implements IRichBolt {
    /**
     *
     */
    private OutputCollector collector = null;
    private static final long serialVersionUID = -5516726617765479099L;
    private Logger log = LoggerFactory.getLogger(LIV_ToUser_Update_Bolt.class);
    private final static String update_new_user = "update liv_viscosity_user set click_num = click_num + 1,playLength = playLength + ? where partner= ? and mac= ? and srcName= ? and chnCode= ? and albumName= ? and playOrder= ? and logDate= ?;";
    ...
    public void execute(Tuple input) {
        try {
            ...
            BoundStatement bs = Mapper_Liv.update_new_user(ps, partner, mac, srcName, chnCode, albumName, playOrder, playLength, lg_date);
            AsyncBack_Liv.update_back(session, bs, collector, input);
        } catch (Exception e) {
            log.debug("LIV_ToUser_Update_Bolt exception:" + e.getMessage());
