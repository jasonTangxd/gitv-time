...
public class VOD_ToUser_Update_Bolt implements IRichBolt {
    /**
     *
     */
    private OutputCollector collector = null;
    private static final long serialVersionUID = -5516726617765479099L;
    private Logger log = LoggerFactory.getLogger(VOD_ToUser_Update_Bolt.class);
    private final static String update_new_user = "update vod_viscosity_user set click_num = click_num + 1,playLength = playLength + ? where partner= ? and mac= ? and srcName= ? and chnName= ? and chnId= ? and albumName= ? and albumId= ? and playOrder= ? and videoId= ? and logDate= ?;";
    ...
    public void prepare(Map stormConf, TopologyContext context, OutputCollector collector) {
        ...
        ps = session.prepare(update_new_user);
    }
    ...
    public void execute(Tuple input) {
        try {
            ...
            BoundStatement bs = Mapper.update_new_user(ps, partner, mac, srcName, chnName, chnId, albumName, albumId, playOrder, videoId, playLength, lg_date);
            AsyncBack.update_back(session, bs, collector, input);
        } catch (Exception e) {
            log.debug("VOD_ToUser_Update_Bolt exception:" + e.getMessage());
