...
public class LIV_ToProgram_Update_Bolt implements IRichBolt {
    ...
    private Logger log = LoggerFactory.getLogger(LIV_ToProgram_Update_Bolt.class);
    private final static String update_new_program = "update liv_viscosity_program set click_num = click_num + 1,playLength = playLength + ? where partner= ? and srcName= ? and chnCode= ? and albumName= ? and playOrder= ? and province= ? and city= ? and logDate= ?;";
    ...
    public void execute(Tuple input) {
        try {
            ...
            BoundStatement bs = Mapper_Liv.update_new_program(ps, partner, srcName, chnCode, albumName, playOrder, province, city, playLength, lg_date);
            AsyncBack_Liv.update_back(session, bs, collector, input);
        } catch (Exception e) {
            log.error("LIV_ToProgram_Update_Bolt exception:" + e.getMessage());
