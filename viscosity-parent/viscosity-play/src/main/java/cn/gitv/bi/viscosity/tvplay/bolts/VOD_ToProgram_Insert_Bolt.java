...
public class VOD_ToProgram_Insert_Bolt implements IRichBolt {
    ...
    private Logger log = LoggerFactory.getLogger(VOD_ToProgram_Insert_Bolt.class);
    private final static String insert_new_program = "update vod_viscosity_program set click_num = click_num + 1,playLength = playLength + ?,timeLength = timeLength + ? where partner= ? and srcName= ? and chnName= ? and chnId= ? and albumName= ? and albumId= ? and playOrder= ? and videoId= ? and province= ? and city= ? and logDate= ?;";
    ...
    public void execute(Tuple input) {
        try {
            ...
            BoundStatement bs = Mapper.insert_new_program(ps, partner, srcName, chnName, chnId, albumName, albumId, playOrder, videoId, province, city, playLength, timeLength, lg_date);
            AsyncBack.update_back(session, bs, collector, input);
        } catch (Exception e) {
            log.error("VOD_ToProgram_Insert_Bolt exception:{}", e.getMessage());
