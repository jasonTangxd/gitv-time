...
public class MultiOutputFormat extends FileOutputFormat<MsgMetadataWritable, BytesWritable> {
    ...
    @Override
    public RecordWriter<MsgMetadataWritable, BytesWritable> getRecordWriter(TaskAttemptContext context) throws IOException {
...
//        final String pathFormat = conf.get(CONFIG_PATH_FORMAT, "'{T}/{P}'");
        /*在这里把path的format固定了*/
        final String pathFormat = conf.get(CONFIG_PATH_FORMAT, "'{T}/'yyyy-MM-dd'/'");
//        log.warn("the CONFIG_PATH_FORMAT Using path format {}", pathFormat);
        ...
        return new RecordWriter<MsgMetadataWritable, BytesWritable>() {
            ...
            public void write(MsgMetadataWritable key, BytesWritable value) throws IOException {
                log.warn("MultiOutputFormat`s MsgMetadataWritable outputKey is {}", key.getTimestamp());
                ...
                /*above is basePath:"'{T}/'yyyy-MM-dd'/'"*/
                String P = String.valueOf(key.getSplit().getPartition());//just use in filename
...
//                suffixPath = suffixPath.replace("{P}", P);
                String fileName = T + "-" + P + "-" + offsetFormat.format(key.getSplit().getStartOffset());
                suffixPath = StringHandle.fullPaht(suffixPath, fileName);
                suffixPath += extension;
                RecordWriter<Void, BytesWritable> recordWriter = this.recordWriters.get(suffixPath);
                try {
