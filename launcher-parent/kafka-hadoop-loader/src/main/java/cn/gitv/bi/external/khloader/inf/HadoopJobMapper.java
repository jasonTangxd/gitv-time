...
package cn.gitv.bi.external.khloader.inf;

import cn.gitv.bi.external.khloader.overwrite.MsgMetadataWritable;
...
public abstract class HadoopJobMapper extends Mapper<MsgMetadataWritable, BytesWritable, MsgMetadataWritable, BytesWritable> {
    ...
    public void map(MsgMetadataWritable key, BytesWritable value, Context context) throws IOException {
        try {
            if (key != null) {
                ...
                //
                Date date = parseDateFromData(outputValue);
                //
...
//                LOG.warn("HadoopJobMap`s MsgMetadataWritable outputKey is {}", outputKey.getTimestamp());
    ...
    public abstract Date parseDateFromData(BytesWritable outputValue);
}