        ...
        try {
            ...
            if ("1".equals(Action)) {
                ...
                if (province == null) {
                    ...
                    collector.emit(LogConst.NOPTP, input, new Values(partner));
                }
                // 2.数据全部发到update_bolt
                collector.emit(LogConst.UPD, input, new Values(type, mac, partner, province, app_version, last_open));
                // 3.里面异步读取合规的数据发送到rabbit bolt
